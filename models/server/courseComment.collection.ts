...
export default async function createCourseCommentCollection() {
  ...
  await databases.createCollection(db, courseCommentCollection, courseCommentCollection, [
  ...
  await Promise.all([
    ...
    databases.createStringAttribute(db, courseCommentCollection, 'created_at', 100, false),
    databases.createStringAttribute(db, courseCommentCollection, 'updated_at', 100, false)
  ]);
}