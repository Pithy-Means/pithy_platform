...
const SingleCourse: React.FC = () => {
  ...
  const [modules, setModules] = useState<Module[]>([
    ...
    },
    {
      ...
      isLocked: true,
      content:
        "To implement the functionality where clicking on a locked module in the sidebar unlocks it and then displays its details in the main content area, you need to:Track the state of locked/unlocked modules.",
    },
    {
  ...
  if (!course) {
    return (
      <p className="flex justify-center items-center font-bold text-4xl py-32 bg-white object-cover text-black h-screen">
        Course not found
      </p>
    );
  }
  ...
  if (loading) {
    return (
      <p className="flex justify-center items-center font-bold text-4xl py-20 bg-white object-cover text-black h-screen">
        Loading...
      </p>
    );
  }
  ...
  const handleUnlockModule = (moduleTitle: string) => {
    setModules((prevModules) =>
      prevModules.map((module) =>
        module.title === moduleTitle ? { ...module, isLocked: false } : module,
      ),
    );
    setActiveModule(moduleTitle);
  };

  const currentModuleContent = modules.find(
    (module) => module.title === activeModule,
  )?.content;
              ...
              <div className="flex items-center space-x-4">
                <div>
                  <p
                    style={{
                      cursor: "pointer",
                      fontWeight: showSummary ? "bold" : "normal",
                    }}
                ...
                <div className="flex flex-col ">
                  <p
                    ...
                    style={{
                      cursor: "pointer",
                      fontWeight: showResources ? "bold" : "normal",
                    }}
