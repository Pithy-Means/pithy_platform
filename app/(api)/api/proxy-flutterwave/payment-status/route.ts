...
export async function GET(req: Request) {
    ...
    try {
        ...
        try {
            const userQuery = await databases.listDocuments(db, userCollection, [Query.equal("email", customerEmail), Query.limit(1000)]);
            logEvent("User query results", { 
                userCount: userQuery.documents.length,
                users: userQuery.documents.map(u => ({ id: u.$id, email: u.email, paid: u.paid, referral_by: u.referral_by }))
            }, 'DEBUG');
            
            if (userQuery.documents.length > 0) {
                const user = userQuery.documents[0];
                // const userUpdatePayload: Record<string, unknown> = {};
                if (!user.paid) user.paid = true;

                if (user.referral_by) { 
                    logEvent(`User ${user.user_id} was referred by someone.`, { referralCode: user.referral_by }, 'DEBUG');
                ...
                if (Object.keys(user).length > 0) {
                    ...
                    await databases.updateDocument(db, userCollection, user.$id, user);
                    logEvent("User record updated successfully", { userId: user.$id, updates: user });
                }
            } else {
