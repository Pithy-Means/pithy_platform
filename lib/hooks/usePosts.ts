...
export const usePosts = () => {
  ...
  const fetchMoreInBackground = useCallback((page: number, limit: number = BATCH_SIZE) => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(`Background fetch completed for page ${page}, got ${result.length} posts`);
          ...
          if (pageRef.current === page) {
            setPosts(prevPosts => {
              ...
              result.forEach(newPost => {
                const existingIndex = uniquePosts.findIndex(p => p.post_id === newPost.post_id);
                if (existingIndex === -1) {
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      ...
      if (cache.current.has(currentPage) && cache.current.get(currentPage)?.length) {
    ...
    const processUpdates = async () => {
      ...
      setPosts(prevPosts => {
        ...
        updates.forEach(update => {
          ...
          if (existingIndex === -1) {
            ...
            JSON.stringify(updatedPosts[existingIndex]) !== JSON.stringify(update)
          ) {
      ...
      updates.forEach(update => {
        cache.current.forEach((posts, page) => {
          const pageUpdated = posts.some(post => post.post_id === update.post_id);
          if (pageUpdated) {
            cache.current.set(
              page,
              posts.map(post => post.post_id === update.post_id ? update : post)
            );
          }
        });
      });
