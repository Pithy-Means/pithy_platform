...
export const fetchPosts = async (page: number = 1, limit: number = 6, signal?: AbortSignal) => {
  ...
  try {
    ...
    const postsWithUserInfo = await Promise.all(
      response.map(async (post) => {
        ...
        const existingPostWithSameUser = currentPosts.find(p => p.user_id === post.user_id);
    ...
    if (postsWithUserInfo.length > 0) {
      ...
      postsWithUserInfo.forEach(newPost => {
        const existingIndex = mergedPosts.findIndex(p => p.post_id === newPost.post_id);
        if (existingIndex >= 0) {
...
export const prefetchPosts = async (startPage: number, numPages: number = 2) => {
  const promises = [];
