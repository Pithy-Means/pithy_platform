...
interface CourseState {
  ...
  setUserCoursePurchase: (userId: string, courseId: string, isPurchased: boolean) => void;
...
const loadPersistedCourseState = (): { userPurchases: Record<string, string[]> } => {
...
export const useCourseStore = create<CourseState>((set, get) => ({
  ...
  setUserCoursePurchase: (userId: string, courseId: string, isPurchased: boolean) => {
    ...
    set((state) => {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem("userCoursePurchases", JSON.stringify(updatedUserPurchases));
      }
  ...
  syncPurchasesFromServer: async (userId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/user/purchased-courses?userId=${userId}`);
      ...
      const purchasedCourseIds = data.courses.map((course: { course_id: string }) => course.course_id);
