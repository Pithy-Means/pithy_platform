...
export default function ProfilePage() {
  const { user } = useContext(UserContext) as { user: UserInfo | null };
            ...
            {user.categories === "student" && (
              <div className="mt-4">
                <p>
                  ...
                  {user.studentInfo?.institution_name || "Not specified"}
                </p>
                <p>
                  <span className="font-semibold">Major Subject:</span>{" "}
                  {user.studentInfo?.major_subject || "Not specified"}
                </p>
                <p>
                  <span className="font-semibold">Graduation Year:</span>{" "}
                  {user.studentInfo?.expected_graduation_year || "Not specified"}
                </p>
              </div>
            )}
            {user.categories === "job seeker" && (
              <div className="mt-4">
                <p>
                  ...
                  {user.jobSeekerInfo?.desired_job_title || "Not specified"}
                </p>
                <p>
                  <span className="font-semibold">Skills:</span>{" "}
                  {user.jobSeekerInfo?.skills || "Not specified"}
                </p>
                <p>
                  <span className="font-semibold">Work Experience:</span>{" "}
                  {user.jobSeekerInfo?.years_of_work_experience
                    ? `${user.jobSeekerInfo.years_of_work_experience} years`
                    : "Not specified"}
                </p>
              </div>
            )}
            {user.categories === "employer" && (
              <div className="mt-4">
                <p>
                  ...
                  {user.employerInfo?.company_name || "Not specified"}
                </p>
                <p>
                  <span className="font-semibold">Company Size:</span>{" "}
                  {user.employerInfo?.company_size || "Not specified"}
                </p>
                <p>
                  <span className="font-semibold">Industry:</span>{" "}
                  {user.employerInfo?.industry_type || "Not specified"}
                </p>
              </div>
            )}
          </>
